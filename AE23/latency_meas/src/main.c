...
//-----------------------------------------------
void print_usage() {
    fprintf(stderr, "[ LOG ] - Usage ./test [-h] [-s repeat] [-r rounds] [-i iteration] [-o o_file] [-v] [--mem mem_size]\n");
    fprintf(stderr, "          -h                     = this help message\n");
    fprintf(stderr, "          -s sets                = number of set repeat per turple    (default: %d)\n", REP_std);
    fprintf(stderr, "          -r rounds              = number of rounds per tuple         (default: %d)\n", ROUNDS_std);
    fprintf(stderr, "          -i iteration           = number of iteration                (default: %d)\n", ITER_std);
...
//size_t rep_cnt, size_t iter_cnt

//-----------------------------------------------
int main(int argc, char** argv) {

    uint64_t    flags       = 0ULL;
    size_t      rep_cnt    = REP_std;
    size_t      rounds      = ROUNDS_std;
    size_t      m_size      = MEM_SIZE_std;
    size_t      iter_cnt   =  ITER_std;
    ...
    while (1) {
        ...
        int arg = getopt_long (argc, argv, "o:s:r:i:hv",
    ...
    rev_mc(rep_cnt, iter_cnt, rounds, m_size, o_file, flags);
    return 0;
